rolador:=StrReplace(A_ScriptName,".ahk","")
rolador:=StrReplace(rolador,".exe","")
rolador .= ".cfg"
if(!FileExist(rolador))
{
	FileAppend,Nome do Personagem`nAtaque Primário`nd20+0`nd6+0`nAtaque Secundário`nd20+0`nd6+0`nOutro`nd20+0`n`nIniciativa`nd20+0`n`n,%rolador%
}
FileRead,config,%rolador%
...
Roll(nome,r)
{
	...
	r%r% := StrReplace(r%r%,"-",";$1-")
	if va%r%
		r%r% := StrReplace(r%r%,";",";0bk1;")
	if de%r%
		r%r% := StrReplace(r%r%,";",";0bkl1;")
	out.= r%r% . ";"
	if d%r%
	{
	...
	out.= "}{"
		...
		if de%r%
			out .= " com Desvantagem"
		if d%r%
			out.= " e para dano $4 ($3) Dano Adicional de Crítico: $5"
	out .= """" . "}"
	}
	out.= "#" . nome
	if va%r% or de%r%
	{
		out := StrReplace(out,"$5","$7")
		out := StrReplace(out,"$4","$6")
		out := StrReplace(out,"$3","$5")
		out := StrReplace(out,"$2","$4")
		out := StrReplace(out,"$1","$3")
		out := StrReplace(out,"($3)","($1,$2)")
		
	}
	if (d%r%)
		out:= StrReplace(out,"Rolagem","Acerto")
	else
		out:= StrReplace(out,"Rolagem","Teste")
...
GuiClose:
gui,submit,nohide
FileAppend,%nome%`n%a21%`n%r21%`n%d21%`n%a22%`n%r22%`n%d22%`n%a23%`n%r23%`n%d23%`n%a24%`n%r24%`n%d24%`n,%rolador%
ExitApp
